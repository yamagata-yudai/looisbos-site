...
### SNSでリンクを共有したときの表示について

X などにURLを貼ったときのカード（OGP）は、**公演名ではなく共通の内容**が出ます。
SNSのクローラーはJavaScriptを実行しないため、`?event=` で切り替わる公演情報を
読み取れないからです。公演ごとにOGPを書き換えると、次の公演のときに古い情報が
残ってしまうので、あえて公演に依存しない内容にしています。

告知ツイートでフライヤー画像を添付する場合は、画像の方が優先して表示されるため
この制約は問題になりません。
